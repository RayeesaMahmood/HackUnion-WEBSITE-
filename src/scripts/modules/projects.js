...
const renderProjectCardVisual = (project, themeStyle) => {
  if (project.image) {
    return `<img class="project-card__image" src="${escapeHtml(project.image)}" alt="${escapeHtml(project.imageAlt || project.title)}" loading="lazy" decoding="async" />`;
  }

  return renderProjectVisual(project, themeStyle);
};
  ...
  return `
    <article class="${cardClasses.join(' ')}" data-project-card data-project-filter="${escapeHtml(project.filter || 'all')}" aria-labelledby="${escapeHtml(project.slug)}-title">
      <a class="project-card__visual" href="${escapeHtml(project.caseStudyUrl || '#')}" aria-label="View project: ${escapeHtml(project.title)}">
        ${renderProjectCardVisual(project, themeStyle)}
...
const initProjectDetailPage = (detailNode) => {
  ...
  const screenshotItems = project.screenshots?.length
    ? project.screenshots
    : [{ image: '../../images/gallery-optimized/2SP00516.jpg', alt: `${project.title} preview` }];
